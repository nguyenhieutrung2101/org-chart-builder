...
vi: {
  ...
  l15d:'Sửa theo review độc lập: chọn CIG rồi đổi mode không còn làm mất luật Chung; dán Excel giữ đúng CBQLNS cũ và liệt kê đủ dòng cần xem trong hộp dán; copy → dán lại giữ nguyên ngoặc kép / xuống dòng; một box ★ chỉ đứng một chỗ trên cây Ngành dọc; logo SVG lọc theo danh sách cho phép; bảng luồng lọc trước khi vẽ và giới hạn số dòng; mọi thay đổi lỗi được rollback; CI chạy test trên mỗi PR.',
  ...
  l12a:'Hàng như kệ sách: kéo box lên/xuống đổi hàng với đường kẻ hướng dẫn; box cao cố định ~5 dòng, chữ dài tự co; nhóm xếp dọc nối rễ trái; ẩn (Tx) riêng từng box; ghi chú chọn từ dropdown.',
  l12b:'Bảng màu gốc văn bản đúng mã; khổ A2 + chiều cao trang tự động; zoom mượt quanh con trỏ và kéo nền không bôi đen chữ.',
  l11a:'Module mới Trình bày sơ đồ: trang in SVG theo mm (A4/A3), tiêu đề, khối mã văn bản, bảng màu cấp, ghi chú chữ cái, định biên, tải PDF nhúng font.',
  l11b:'Màn hình chọn module; cấp <b>ĐB</b> trên CC; một cây tổ chức dùng chung cho cả hai module.',
  l11c:'Tách mã nguồn thành từng file theo section; bộ test Playwright đi kèm repo.',
  ...
  msgPdfDoneLocal:'Đã tạo PDF — nhúng {f} từ máy bạn', msgPdfDoneSubst:'Đã tạo PDF — trình duyệt không cho lấy {f}, dùng Liberation cùng metric',
  fontHint:'PDF nhúng font này lấy từ máy bạn (Chrome/Edge hỏi quyền một lần); không lấy được thì dùng Liberation cùng metric.',
  ...
  msgImportNotes:'{n} dòng cần xem lại — danh sách trong hộp dán',
  impAmbCb:"'{name}': {n} box ★ trùng tên", impNoCb:"'{name}': không có box ★ tên này",
  impBlank:' — chưa gán CBQLNS', impKept:' — giữ nguyên CBQLNS hiện tại ({cur})', impReviewH:'Dòng cần xem lại ({n}):',
  resCapNote:'Hiện {n} / {total} dòng — gõ từ khoá lọc để thu hẹp; Copy bảng luồng vẫn xuất đủ.',
  msgMutateFail:'Thao tác bị lỗi, đã khôi phục trạng thái trước đó',
  ...
  errOrgTwice:'box {id} nằm ở hai chỗ trên cây ngành dọc ({a} / {b})', vRootLabel:'(gốc)',
  errTsvQuote:'Ngoặc kép mở ở dòng {line} chưa đóng — chưa nhập gì',
  ...
  btnPasteGo:'Nhập', btnPasteCancel:'Hủy', ckPasteHdr:'Dòng đầu là tiêu đề (bỏ qua)',
...
en: {
  ...
  l15d:'Fixes from an independent review: selecting a CIG then switching mode no longer loses the Common rules; Excel paste keeps the existing BMO and lists every row to review inside the paste box; copy → paste keeps quotes / line breaks; a ★ box appears once on the vertical tree; SVG logo filtered by an allowlist; the result table filters before drawing and caps rows; failed changes roll back; CI runs the tests on every PR.',
  ...
  l12a:'Shelf rows: drag boxes up/down between rows with guide lines; fixed ~5-line box height with auto-shrinking text; stacked groups on a left spine; per-box (Tx) toggle; note key picked from a dropdown.',
  l12b:'Exact document palette; A2 paper + auto page height; smooth zoom around the cursor and drag-to-pan without selecting text.',
  l11a:'New Chart layout module: printable SVG page in mm (A4/A3), header, document-code block, level legend, lettered notes, headcount, PDF with embedded fonts.',
  l11b:'Module chooser; new <b>ĐB</b> level above CC; one org tree shared by both modules.',
  l11c:'Source split into one file per section; Playwright test suite ships with the repo.',
  ...
  msgPdfDoneLocal:'PDF created — {f} embedded from your computer', msgPdfDoneSubst:"PDF created — the browser would not hand over {f}, metric-compatible Liberation used instead",
  fontHint:'The PDF embeds this font from your computer (Chrome/Edge asks once); otherwise metric-compatible Liberation is used.',
  ...
  msgImportNotes:'{n} rows to review — listed in the paste box',
  impAmbCb:"'{name}': {n} ★ boxes share this name", impNoCb:"'{name}': no ★ box with this name",
  impBlank:' — BMO left blank', impKept:' — existing BMO kept ({cur})', impReviewH:'Rows to review ({n}):',
  resCapNote:'Showing {n} of {total} rows — type a filter to narrow down; Copy still exports everything.',
  msgMutateFail:'The change failed and was rolled back',
  ...
  errOrgTwice:'box {id} appears twice on the vertical tree ({a} / {b})', vRootLabel:'(root)',
  errTsvQuote:'Unclosed quote opened on line {line} — nothing imported',
  ...
  btnPasteGo:'Import', btnPasteCancel:'Cancel', ckPasteHdr:'First row is a header (skip it)',
